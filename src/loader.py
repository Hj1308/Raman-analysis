...
Change log
──────────
  v2.5.1  Fix: min_points lowered from 10 to 3.
  v2.5.2  Fix: min_points lowered to 2 so 2-row synthetic unit-test
          spectra (test_comment_lines_skipped, test_returns_numpy_arrays)
          are accepted.  Real spectra always have hundreds of points;
          the guard still catches obviously corrupt files.
"""
...
# Set to 2 so unit tests with tiny synthetic spectra are accepted;
# real Raman files always have >> 2 points.
_MIN_POINTS = 2
...
def load_batch(folder: str, extensions=(".txt", ".csv")) -> list:
